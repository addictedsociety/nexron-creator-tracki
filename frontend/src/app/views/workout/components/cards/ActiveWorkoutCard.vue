  ...
  <Card class="w-full border-primary shadow-lg shadow-indigo-500/20" v-if="w.isWorkoutActive">
    ...
    <CardContent class="space-y-4">
      ...
      <div class="flex gap-2 items-center justify-center flex-wrap">
        <Button @click="w.pauseWorkout" variant="outline" size="sm">
          <Pause class="h-4 w-4 mr-2" />Pausieren
        </Button>
        <Button @click="w.resumeWorkout" variant="outline" size="sm">
          <Play class="h-4 w-4 mr-2" />Fortsetzen
        </Button>
        <Button @click="w.finishWorkout" variant="destructive" size="sm">
          <Square class="h-4 w-4 mr-2" />Beenden
        </Button>
      </div>
      ...
      <div class="space-y-3" v-if="(w.currentWorkout?.exercises ?? []).length > 0">
        ...
        <div class="space-y-3">
          <ExerciseCard v-for="ex in (w.currentWorkout?.exercises ?? [])" :key="ex.id" :exercise="ex"
            @remove="w.removeExercise(ex.id)" @toggle="(setId: string) => w.toggleSetCompletion(ex.id, setId)"
